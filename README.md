...
主界面是三栏浏览器：侧边栏按范围和 Agent 分组，中间是可搜索、可排序的 Skill 列表，右侧显示选中 Skill 的详情——简介、frontmatter、渲染后的 Markdown 文档、关联的 Agents 和安装位置。搜索时普通词匹配 Skill 名称、简介或已索引正文；给词加上 `name:`、`desc:`、`path:`、`agent:`、`body:` 前缀就只搜对应字段，比如 `agent:cursor path:.agents`。顶栏左侧有前进 / 后退（⌘[ / ⌘]），菜单栏「前往」也有同样的前进 / 后退，按 ⌘F 聚焦搜索框；侧边栏切换、打开详情、搜索各记一步历史。中列右缘可以拖拽调整宽度，应用是单窗口（菜单里没有"新建窗口"）。
