...
2. 校验完整性（两个文件放同一目录）：
   ...
   输出必须是 `SkillSelector-1.01.dmg: OK`。不是就别装。
...
产物：`dist/SkillSelector.app`（通用 2）与 `dist/SkillSelector-arm64.app`、`dist/SkillSelector-x86_64.app`，三个 DMG（`SkillSelector.dmg`、`SkillSelector-1.01.dmg` 及两份单架构）和对应的 `.sha256`。
