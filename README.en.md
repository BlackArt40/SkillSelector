...
2. Verify integrity (keep both files in the same directory):
   ...
   It must print `SkillSelector-1.02.dmg: OK`. If it doesn't, don't install it.
...
Produces `dist/SkillSelector.app` (Universal 2) plus `dist/SkillSelector-arm64.app` and `dist/SkillSelector-x86_64.app`, three DMGs (`SkillSelector.dmg`, `SkillSelector-1.02.dmg`, and the two single-arch ones), and matching `.sha256` files.
