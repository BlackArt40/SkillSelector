...
2. Verify integrity (keep both files in the same directory):
   ...
   ```zsh
   shasum -a 256 -c SkillSelector-1.6.1.dmg.sha256
   ```

   It must print `SkillSelector-1.6.1.dmg: OK`. If it doesn't, don't install it.
...
```zsh
swift build
zsh Scripts/package-dmg.sh 1.6.1
```

Produces `dist/SkillSelector.app`, `dist/SkillSelector.dmg`, `dist/SkillSelector-1.6.1.dmg`, and a matching `.sha256`.
