...
This document records the release checks for the confirmed product specification. Automated checks use an isolated temporary directory; they do not read a real home directory, launch installed `gh` or `npm`, access the network, or invoke the macOS Trash.
...
`AcceptanceTests` creates a unique directory below the system temporary directory, including a disposable home, nested projects, fake executables, a fake stdio MCP server, a URLProtocol-backed HTTP MCP server, and an injected Trash adapter.
...
| Refresh and descriptions | Manual refresh retains an inaccessible project record as unavailable; local descriptions override enrichment and a custom description overrides local content. |
...
| Enrichment | Fake `gh` and `npm` receive only expected read-only argument arrays. A GitHub candidate requires explicit source-binding confirmation. |
| MCP safety | Disabled servers do not run; stdio requires exact-command approval, configuration changes invalidate approval, only the enabled read-only tool is invoked, package runners remain unlaunched, and HTTP traffic is intercepted by an in-process URL protocol. |
